...
genuinely lossless (bit-for-bit recoverable pixels), not just high-quality
lossy — `exact: 1` additionally preserves RGB values under fully
transparent pixels, which plain PNG also does but WebP's lossy path
doesn't bother with.
...
*(pending — see the Audio tab's WAV→FLAC convert-format suggestion)*
